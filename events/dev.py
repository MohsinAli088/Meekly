...
class DevTools(commands.Cog):
    ...
    # 🔄 Sync Emojis Command
    # ======================
    @commands.command(name="syncemojis", aliases=["syncemoji", "se"])
    @is_dev()
    async def sync_emojis_command(self, ctx, force: bool = False):
        """Sync emojis from emoji/assets/ with Discord Developer Portal (Application Emojis).
        Usage: !syncemojis [force=true/false]
        """
        msg = await ctx.send("🔄 Syncing emojis with Discord Developer Portal (Application Emojis)...")
        try:
            from emojis import sync_application_emojis
            synced, total = await sync_application_emojis(self.bot, force_reupload=force)
            await msg.edit(content=f"{EMOJI_CHECK} Successfully synced `{synced}/{total}` emojis directly from Discord Developer Portal!")
        except Exception as e:
            await msg.edit(content=f"{EMOJI_CROSS} Emoji sync failed: `{e}`")
