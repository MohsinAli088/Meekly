...
from emojis import CHECK, CROSS_MARK, REPLY_2, ZTICK, UNGHOST, LOADING, TICK, VERIFIED_CHECK
...
class Welcomer(commands.Cog):
    ...
    async def greet_setup(self, ctx):
        ...
        if row:
            ...
            error.set_author(name="Error", icon_url="https://cdn.discordapp.com/emojis/1408118547086835904.png")
    ...
    async def simple_setup(self, ctx):
        ...
        async def submit_callback(interaction):
            ...
            if message_content:
                ...
                await interaction.response.send_message(f"{CHECK} Welcome message setup completed!")
        ...
        cancel_button = Button(emoji=LOADING, style=discord.ButtonStyle.secondary)
    ...
    color=discord.Color(embed_data["color"]) if embed_data["color"] else discord.Color(0x2f3136)
        ...
        async def submit_callback(interaction):
            ...
            await interaction.response.send_message(f"{ZTICK} Embed welcome message setup completed!")
    ...
    async def greet_reset(self, ctx):
        ...
        if not is_set_up: 
            ...
            error.set_author(name="Greet is not configured!", icon_url="https://cdn.discordapp.com/emojis/1408118547086835904.png")
            return await ctx.send(embed=error)
        ...
        async def yes_button_callback(interaction):
            ...
            embed.title = f"{VERIFIED_CHECK} Success"
    ...
    async def greet_channel(self, ctx):
        ...
        def generate_view(page):
            select_menu = Select(
                ...
                options=[
                    discord.SelectOption(label=channel.name, emoji=UNGHOST, value=str(channel.id))
            ...
            async def select_callback(interaction: discord.Interaction):
                ...
                await ctx.send(f"{VERIFIED_CHECK} Welcome channel has been set to {selected_channel.mention}")
    ...
    async def greet_test(self, ctx):
        ...
        if not welcome_channel:
            ...
            error2.set_author(name="Channel not set", icon_url="https://cdn.discordapp.com/emojis/1408118547086835904.png")
    ...
    async def greet_autodelete(self, ctx, time: str):
        ...
        await ctx.send(f"{VERIFIED_CHECK} Auto delete duration has been set to **{auto_delete_duration}** seconds.")
